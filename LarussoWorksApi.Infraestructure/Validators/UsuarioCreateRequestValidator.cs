...
namespace LarussoWorksApi.Infraestructure.Validators
{
    ...
    public class UsuarioCreateRequestValidator : AbstractValidator<UsuarioCreateRequest>
    {
        public UsuarioCreateRequestValidator()
        {
            ...
            RuleFor(x => x.CorreoUsuario).NotEmpty().WithMessage("El Correo, no debe estar vacio")
                .EmailAddress().WithMessage("Se requiere una dirección de correo electronico válido (correo@mail.com)");

            RuleFor(x => x.ContrasenaUsuario)
                ...
                .Must(x => x.Length < 20).WithMessage("La contraseña, debe tener menos de 10 caracteres");
